...
summary(fit)
summary(fit_aic)
# smaller summary p value means that H0(The model doesnt fit) is lesser to happen
# model is better more if p value is smaller
# model fit is based on the r sqrd
...
fit_wine.test = lm(quality ~ . -total.sulfur.dioxide, wine.test)
summary(fit_wine.test)
